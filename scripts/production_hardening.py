...
replace_once(feed, "import { apiJson } from '../../utils/socialApi';\n", "import { apiJson } from '../../utils/socialApi';\nimport { prefetchMedia } from '../../utils/mediaCache';\n")
replace_once(feed, "  const [statusMessage, setStatusMessage] = useState('');\n", "  const [statusMessage, setStatusMessage] = useState('');\n  const [feedOffset, setFeedOffset] = useState(0);\n  const [loadingMore, setLoadingMore] = useState(false);\n")
replace_once(feed, "query: { limit: 20 },", "query: { limit: 5, offset: 0 },")
replace_once(feed, "          setPosts(mappedPosts);\n\n          for (const item of mappedPosts) {", "          setPosts(mappedPosts);\n          setFeedOffset(mappedPosts.length);\n          void prefetchMedia(mappedPosts.slice(0, 3).map((item) => item.image_url ?? '').filter(Boolean));\n\n          for (const item of mappedPosts) {")
...
# deterministic trigger for the migration workflow
