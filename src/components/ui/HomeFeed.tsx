...
export function HomeFeed({ onInteraction }: { onInteraction: (type: string, tag: string) => void }) {
    ...
    const loadPosts = async () => {
      try {
        ...
        if (data?.success) {
          ...
          void prefetchMedia(mappedPosts.slice(0, 5).map((item) => item.image_url ?? '').filter(Boolean), 5);
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      void prefetchMedia(next.slice(0, 5).map((item) => item.image_url ?? '').filter(Boolean), 5);
    } finally {
