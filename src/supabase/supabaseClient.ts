...
async function request(path: string, init: RequestInit = {}, token?: string) {
  requireConfig(); const headers = new Headers(init.headers || {}); headers.set('apikey', SUPABASE_KEY); headers.set('Content-Type', 'application/json'); if (token) headers.set('Authorization', `Bearer ${token}`);
  const response = await fetch(`${SUPABASE_URL}${path}`, { ...init, headers }); let body: any = null; try { body = await response.json(); } catch { /* empty response */ }
  if (!response.ok) { const error: any = new Error(body?.msg || body?.message || body?.error_description || body?.error || `Supabase request failed (${response.status})`); error.status = response.status; error.code = body?.code; throw error; }
  return body;
}
...
async function initialize() {
  ...
  try { const stored = window.localStorage.getItem(STORAGE_KEY); currentSession = stored ? JSON.parse(stored) : null; } catch { currentSession = null; }
  ...
  if (code) {
    ...
    if (verifier) {
      try {
        const result = await request('/auth/v1/token?grant_type=pkce', { method: 'POST', body: JSON.stringify({ auth_code: code, code_verifier: verifier }) });
        const session: Session = { ...result, expires_at: Math.floor(Date.now() / 1000) + Number(result.expires_in || 3600) };
        save(session); window.sessionStorage.removeItem(PKCE_KEY); url.searchParams.delete('code'); url.searchParams.delete('state'); window.history.replaceState({}, document.title, url.pathname + (url.search ? url.search : ''));
        emit('SIGNED_IN', session);
      } catch (error) { window.sessionStorage.removeItem(PKCE_KEY); emit('AUTH_ERROR', null); throw error; }
    }
  }

  const hash = new URLSearchParams(window.location.hash.replace(/^#/, ''));
  const accessToken = hash.get('access_token'); const refreshToken = hash.get('refresh_token'); const expiresIn = Number(hash.get('expires_in') || 3600); const type = hash.get('type');
  if (accessToken && refreshToken) {
    try { const user = await userForToken(accessToken); const session: Session = { access_token: accessToken, refresh_token: refreshToken, expires_in: expiresIn, expires_at: Math.floor(Date.now() / 1000) + expiresIn, user }; save(session); window.history.replaceState({}, document.title, window.location.pathname + window.location.search); emit(type === 'recovery' ? 'PASSWORD_RECOVERY' : 'SIGNED_IN', session); } catch { save(null); }
  } else if (currentSession) emit('INITIAL_SESSION', currentSession); else emit('INITIAL_SESSION', null);
}
void initialize();

const auth = {
  async getSession() {
    await initialize(); if (!currentSession) return { data: { session: null }, error: null };
    const expiresAt = currentSession.expires_at || 0;
    if (expiresAt && expiresAt - Math.floor(Date.now() / 1000) < 60) {
      try { const refreshed = await request('/auth/v1/token?grant_type=refresh_token', { method: 'POST', body: JSON.stringify({ refresh_token: currentSession.refresh_token }) }); currentSession = { ...currentSession, ...refreshed, expires_at: Math.floor(Date.now() / 1000) + Number(refreshed.expires_in || 3600) }; if (!currentSession.user) currentSession.user = await userForToken(currentSession.access_token); save(currentSession); emit('TOKEN_REFRESHED', currentSession); }
      catch { save(null); emit('SIGNED_OUT', null); return { data: { session: null }, error: null }; }
    }
...
function queryBuilder(table: string) {
  let columns = '*'; let filters: Array<[string, string]> = []; let updatePayload: Record<string, unknown> | null = null;
  const builder: any = {
