...
function requireConfig() {
  if (!SUPABASE_URL || !SUPABASE_KEY) {
    throw new Error('Supabase is not configured for this deployment. Set VITE_SUPABASE_URL and VITE_SUPABASE_PUBLISHABLE_KEY (or VITE_SUPABASE_ANON_KEY) in the Vercel Production environment.');
  }
}
...
const auth = {
  async getSession() {
    ...
    if (expiresAt && expiresAt - Math.floor(Date.now() / 1000) < 60) {
      try {
        ...
        const refreshedSession: Session = { ...session, ...refreshed, expires_at: Math.floor(Date.now() / 1000) + Number(refreshed.expires_in || 3600) };
  ...
  async signInWithOtp({ phone }: { phone: string; options?: any }) {
    try { await request('/auth/v1/otp', { method: 'POST', body: JSON.stringify({ phone, create_user: true }) }); return { data: {}, error: null }; }
  ...
  async resetPasswordForEmail(email: string, options: { redirectTo: string }) {
    try { await request('/auth/v1/recover', { method: 'POST', body: JSON.stringify({ email, redirect_to: options.redirectTo }) }); return { data: {}, error: null }; }
    catch (error: any) { return { data: {}, error }; }
  },
  async updatePassword(password: string) { return auth.updateUser({ password }); },
...
function queryBuilder(table: string) {
  ...
  const builder: any = {
    ...
    insert(value: Record<string, unknown> | Record<string, unknown>[]) { insertPayload = value; return builder; },
    async maybeSingle() {
      requireConfig();
    ...
    then(resolve: any, reject: any) {
      const run = async () => {
        ...
        for (const [key, value] of filters) params.set(key, `eq.${value}`);
        const requestInit: RequestInit = insertPayload ? { method: 'POST', body: JSON.stringify(insertPayload) } : updatePayload ? { method: 'PATCH', body: JSON.stringify(updatePayload) } : {};
        const result = await request(`/rest/v1/${table}?${params.toString()}`, requestInit, currentSession?.access_token);
