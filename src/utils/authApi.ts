...
export async function signupWithWizard(params: { username: string; email: string; password: string; phone_country_code?: string; phone_number?: string; legal_accepted?: boolean }): Promise<AuthSuccessResponse> { return completeSignup({ username: params.username, email: params.email, password: params.password, phone: `${params.phone_country_code || '+1'}${String(params.phone_number || '').replace(/\D/g, '')}`, legalAccepted: Boolean(params.legal_accepted) }); }
