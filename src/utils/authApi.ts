...
function toAuthError(error: { message?: string; status?: number; code?: string } | null): Error {
  ...
  if ((lower.includes('invalid') && lower.includes('otp')) || lower.includes('token has expired')) return new Error('The OTP is incorrect or expired.');
...
export async function verifySignupOtp(phone: string, otpCode: string): Promise<AuthSuccessResponse> {
  ...
  if (error || !data.session || !data.user) throw toAuthError(error);
  return { success: true, access_token: data.session.access_token, refresh_token: data.session.refresh_token, expires_in: data.session.expires_in || 3600, user: { id: data.user.id, username: String(data.user.user_metadata?.username || data.user.phone || 'user'), phone: data.user.phone } };
}

export async function checkSignupAvailability(_email: string, username: string): Promise<{ email_available: boolean; username_available: boolean }> {
  const normalizedUsername = validateUsername(username);
  const { data, error } = await supabase.from('profiles').select('user_id,username').eq('username', normalizedUsername).maybeSingle();
  if (error) throw new Error('Unable to validate username right now.');
  // Supabase Auth intentionally does not expose auth.users to the browser. Email uniqueness is enforced authoritatively by Auth during account completion.
...
export async function completeSignup(params: { username: string; email: string; password: string; phone: string; legalAccepted: boolean }): Promise<AuthSuccessResponse> {
  ...
  if (!availability.username_available) throw new Error('Username is already taken.');
  ...
  if (profileError) {
    if (String(profileError.code || '').includes('23505')) throw new Error('Username is already taken.');
    throw new Error('Account profile could not be completed. Please try again.');
  }
  const session = await getCurrentSession();
  if (!session) throw new Error('Account was created but the session could not be established. Please log in.');
...
export async function signupWithWizard(params: { username: string; email: string; password: string; phone_country_code?: string; phone_number?: string; legal_accepted?: boolean }): Promise<AuthSuccessResponse> {
  return completeSignup({ username: params.username, email: params.email, password: params.password, phone: `${params.phone_country_code || '+1'}${String(params.phone_number || '').replace(/\D/g, '')}`, legalAccepted: Boolean(params.legal_accepted) });
}
...
export async function updatePassword(password: string): Promise<void> {
  ...
  const { error } = await supabase.auth.updatePassword(password);
  if (error) throw toAuthError(error);
}
