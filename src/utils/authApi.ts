...
export async function verifySignupOtp(phone: string, otpCode: string): Promise<AuthSuccessResponse> {
  ...
  if (error || !data.session || !data.user) throw toAuthError(error);
  return { success: true, access_token: data.session.access_token, refresh_token: data.session.refresh_token, expires_in: data.session.expires_in || 3600, user: { id: data.user.id, email: data.user.email, username: String(data.user.user_metadata?.username || data.user.phone || 'user'), phone: data.user.phone } };
}
...
export async function checkSignupAvailability(email: string, username: string): Promise<{ email_available: boolean; username_available: boolean }> {
  ...
  return { username_available: !data, email_available: Boolean(normalizedEmail) };
}
...
export async function completeSignup(params: { username: string; email: string; password: string; phone: string; legalAccepted: boolean }): Promise<AuthSuccessResponse> {
  ...
  const { data: userData, error: userError } = await supabase.auth.updateUser({
    ...
    data: { username, mobile: params.phone, terms_accepted: true, privacy_accepted: true, legal_accepted_at: new Date().toISOString() },
  });
...
export async function signupWithWizard(params: any): Promise<AuthSuccessResponse> {
