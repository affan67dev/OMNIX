...
function validateEmail(value: string): string { const email = value.trim().toLowerCase(); if (!/^\S+@\S+\.\S+$/.test(email)) throw new Error('Enter a valid email address.'); return email; }
function toAuthError(error: { message?: string; status?: number; code?: string } | null): Error { const message = error?.message || 'Authentication request failed.'; const lower = message.toLowerCase(); if (lower.includes('rate') || error?.status === 429) return new Error('Too many attempts. Please wait and try again.'); if ((lower.includes('invalid') && lower.includes('otp')) || lower.includes('token has expired')) return new Error('The OTP is incorrect or expired.'); if (lower.includes('expired')) return new Error('The OTP has expired. Request a new code.'); if (lower.includes('already') || lower.includes('duplicate') || lower.includes('unique')) return new Error('That account information is already in use.'); if (lower.includes('weak password')) return new Error('Choose a stronger password.'); return new Error(message); }
...
export async function completeSignup(params: { username: string; email: string; password: string; phone: string; legalAccepted: boolean }): Promise<AuthSuccessResponse> {
  ...
  const username = validateUsername(params.username); const email = validateEmail(params.email); validatePassword(params.password);
  ...
  const legalVersion = '2026-07-21';
  const { error: legalError } = await supabase.from('legal_consents').insert([
    { user_id: userData.user.id, document_type: 'terms', document_version: legalVersion, source: 'signup' },
    { user_id: userData.user.id, document_type: 'privacy', document_version: legalVersion, source: 'signup' },
  ]);
  if (legalError && !String(legalError.code || '').includes('23505')) throw new Error('Legal consent could not be recorded. Please try again.');
  ...
  const response = { success: true, access_token: session.access_token, refresh_token: session.refresh_token, expires_in: session.expires_in || 3600, user: { id: userData.user.id, email: userData.user.email, username, phone: userData.user.phone || params.phone } };
  persistAuthSession(response); return response;
}

export async function signupWithWizard(params: { username: string; email: string; password: string; phone_country_code?: string; phone_number?: string; legal_accepted?: boolean; first_name?: string; last_name?: string; display_nickname?: string; mobile?: string; otp_challenge_id?: string }): Promise<AuthSuccessResponse> { return completeSignup({ username: params.username, email: params.email, password: params.password, phone: `${params.phone_country_code || '+1'}${String(params.phone_number || params.mobile || '').replace(/\D/g, '')}`, legalAccepted: Boolean(params.legal_accepted) }); }
...
export async function requestPasswordReset(email: string): Promise<void> { const value = validateEmail(email); const { error } = await supabase.auth.resetPasswordForEmail(value, { redirectTo: `${window.location.origin}/forgot-password` }); if (error) throw toAuthError(error); }
export async function updatePassword(password: string): Promise<void> { validatePassword(password); const { error } = await supabase.auth.updatePassword(password); if (error) throw toAuthError(error); }