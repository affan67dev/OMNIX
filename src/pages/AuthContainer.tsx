...
export function AuthContainer() {
  ...
  const clearCorruptLocalState = () => {
    try {
      ...
      console.error('Failed to clear local authentication state', error);
    }
  };
    ...
    const bootstrap = async () => {
      try {
        ...
        try {
          ...
          if (sessionUser) {
            const usernameValue = String(sessionUser.user_metadata?.username || sessionUser.email?.split('@')[0] || sessionUser.phone || 'omnix_user');
        ...
        setBackendWarning(apiConfigurationError || (!backendReachable ? 'Backend service is unavailable. Authentication still uses Supabase, but some application features may be unavailable.' : ''));

        try {
          if (await hasConfiguredAppLock()) setAppLocked(true);
        } catch (error) {
          console.error('App lock bootstrap read failed', error);
        }

        if (restoredSession && launchPayload?.targetScreen === 'chat') {
          setActiveTab('messages'); setTargetConversationId(launchPayload.conversationId ?? '');
        } else if (restoredSession && launchPayload?.targetScreen === 'profile') {
          setActiveTab('profile');
        } else if (restoredSession && launchPayload?.targetScreen === 'settings') {
        ...
        if (backendReachable && pendingPushToken) {
          try {
            await apiJson('/api/v1/push/register-device', {
              method: 'POST',
              body: JSON.stringify({ fcm_token: pendingPushToken, platform: 'android', device_id: 'native-webview-shell', app_version: '1.0.0' }),
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      ...
      void checkSignupAvailability(trimmedEmail, trimmedUsername)
        ...
        .catch(() => { setAvailabilityChecked(false); setAvailabilityStatus('Unable to validate username right now. Check your network and retry.'); })
  ...
  const resetSession = async () => {
    try { await supabase.auth.signOut(); } catch (error) { console.error('Supabase sign-out failed', error); }
    clearCorruptLocalState();
    setScreen('login'); setActiveTab('home'); setTargetConversationId(''); setIdentifier(''); setPassword(''); setCurrentUsername('omnix_user'); setAuthError(''); setAuthBusy(false); setOfflineMode(Boolean(apiConfigurationError));
    setSignupStep(1); setCountryCode('+91'); setPhoneNumber(''); setOtpChallengeId(''); setOtpCode(''); setOtpVerified(false); setEmail(''); setUsername(''); setDisplayNickname(''); setAvailabilityChecked(false); setAvailabilityStatus(''); setSignupPassword(''); setConfirmPassword(''); setLegalAccepted(false); setAlgoLogs(['Algorithm Initialized: Passive State Listening...']);
  };
  ...
  const triggerPostPermission = () => {
    ...
    if (userConfirm) alert('✅ Storage pipeline connection authorized without leaking system location strings.');
  };
  ...
  if (screen === 'dashboard') {
    ...
    return <div style={{ background: '#000', color: '#fff', minHeight: '100vh', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', paddingBottom: showBottomBar ? 54 : 0 }}>
      {backendWarning && <div role="status" style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 500, padding: '8px 12px', background: '#3f2a00', color: '#fde68a', fontSize: 12, textAlign: 'center' }}>{backendWarning}</div>}
      {activeTab !== 'messages' && activeTab !== 'settings' && <div style={{ height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px', borderBottom: '1px solid #121212', position: 'sticky', top: 0, zIndex: 100 }}><span style={{ fontSize: 22, fontWeight: 'bold', fontStyle: 'italic' }}>OMNIX</span><span style={{ fontSize: 22 }}>♡　➤</span></div>}
      ...
      <div style={{ display: activeTab === 'home' ? 'block' : 'none' }}><Stories username={targetUsername} /><HomeFeed onInteraction={registerAlgorithmicTrain} /></div>
      <div style={{ display: activeTab === 'search' ? 'block' : 'none', padding: 16 }}><input placeholder="Search secure metadata pools..." style={{ width: '100%', padding: '10px 14px', borderRadius: 8, background: '#121212', border: '1px solid #262626', color: '#fff' }} /></div>
      ...
      {showBottomBar && <div style={{ height: 50, background: '#000', borderTop: '1px solid #121212', position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'space-around' }}><button onClick={() => setActiveTab('home')}>⌂</button><button onClick={() => setActiveTab('search')}>⌕</button><button onClick={triggerPostPermission}>＋</button><button onClick={() => setActiveTab('reels')}>▶</button><button onClick={() => setActiveTab('profile')}>◯</button></div>}
      <LegalDocumentModal open={Boolean(legalModal)} type={legalModal ?? 'terms'} onClose={() => setLegalModal(null)} />
    </div>;
  }

  const finalizeAuth = (usernameValue: string) => { setCurrentUsername(usernameValue || 'omnix_user'); setIdentifier(usernameValue || 'omnix_user'); setScreen('dashboard'); setActiveTab('home'); setAuthError(''); setOfflineMode(Boolean(apiConfigurationError)); };
  ...
  return <div style={{ background: 'linear-gradient(130deg, #f0f9ff 0%, #ecfeff 34%, #fdf2f8 68%, #fef9c3 100%)', color: '#111827', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between', padding: '30px 20px', boxSizing: 'border-box' }}>
    <div style={{ marginTop: 8, textAlign: 'center' }}><h1 style={{ fontSize: 34, fontWeight: 900, letterSpacing: '0.08em', margin: 0 }}>OMNIX</h1><p style={{ margin: '6px 0 0', color: '#475569', fontSize: 13 }}>OMNIX: Secure social experience</p></div>
    <div style={{ width: '100%', maxWidth: 420 }}>
      <div style={{ background: 'rgba(255,255,255,.9)', backdropFilter: 'blur(14px)', border: '1px solid #e2e8f0', padding: 24, borderRadius: 24, boxShadow: '0 20px 50px rgba(30,41,59,.12)' }}>
        ...
        {screen === 'login' ? <form onSubmit={handleLogin}><h2 style={{ margin: '0 0 12px', fontSize: 22, fontWeight: 800 }}>Login</h2><input value={identifier} onChange={(event) => setIdentifier(event.target.value)} required placeholder="Identifier (email or phone)" style={{ width: '100%', padding: 12, borderRadius: 12, border: '1px solid #cbd5e1', marginBottom: 12, boxSizing: 'border-box' }} /><input type="password" value={password} onChange={(event) => setPassword(event.target.value)} required placeholder="Password" style={{ width: '100%', padding: 12, borderRadius: 12, border: '1px solid #cbd5e1', marginBottom: 14, boxSizing: 'border-box' }} /><button type="button" onClick={() => { window.location.href = '/forgot-password'; }} style={{ border: 'none', background: 'transparent', color: '#0369a1', cursor: 'pointer', textDecoration: 'underline', fontSize: 13, padding: 0, marginBottom: 14 }}>Forgot Password?</button><button type="submit" disabled={authBusy} style={{ width: '100%', padding: 12, borderRadius: 12, border: 'none', background: '#0f172a', color: '#fff', fontWeight: 700, opacity: authBusy ? .7 : 1 }}>{authBusy ? 'Signing in…' : 'Login'}</button><div style={{ marginTop: 14, textAlign: 'center', fontSize: 13, color: '#475569' }}>New here? <button type="button" onClick={() => { setScreen('signup'); setAuthError(''); }} style={{ border: 'none', background: 'transparent', color: '#0369a1', textDecoration: 'underline', cursor: 'pointer', padding: 0, fontWeight: 700 }}>Sign Up</button></div></form> : <form onSubmit={createAccount}><button type="button" onClick={() => { setScreen('login'); setAuthError(''); }} style={{ borderRadius: 999, border: '1px solid #cbd5e1', background: '#fff', color: '#334155', padding: '8px 12px', fontWeight: 700 }}>Back to Login</button><div style={{ fontSize: 12, color: '#0369a1', textTransform: 'uppercase', letterSpacing: '.14em', margin: '10px 0 8px' }}>Step {signupStep} of 3</div>{signupStep === 1 && <div><h2 style={{ margin: '0 0 8px', fontSize: 20 }}>What is your phone number?</h2><div style={{ display: 'grid', gridTemplateColumns: '100px 1fr', gap: 8, marginBottom: 10 }}><input value={countryCode} onChange={(event) => setCountryCode(event.target.value)} placeholder="+91" style={{ padding: 12, borderRadius: 12, border: '1px solid #cbd5e1' }} /><input value={phoneNumber} onChange={(event) => setPhoneNumber(event.target.value)} placeholder="Please enter your phone number" style={{ padding: 12, borderRadius: 12, border: '1px solid #cbd5e1' }} /></div><button type="button" onClick={() => void requestOtp()} disabled={authBusy} style={{ width: '100%', padding: 12, borderRadius: 12, border: 'none', background: '#0f172a', color: '#fff', fontWeight: 700 }}>{authBusy ? 'Sending OTP…' : 'Send OTP'}</button>{otpChallengeId && <div style={{ marginTop: 14 }}><OtpInput onComplete={(value) => setOtpCode(value)} /><button type="button" onClick={() => void verifyOtp()} disabled={authBusy || otpCode.length !== 6} style={{ width: '100%', marginTop: 10, padding: 12, borderRadius: 12, border: '1px solid #94a3b8', background: '#f8fafc', color: '#0f172a', fontWeight: 700 }}>{authBusy ? 'Verifying…' : 'Verify OTP'}</button></div>}</div>}{signupStep === 2 && <div><h2 style={{ margin: '0 0 8px', fontSize: 20 }}>Profile Identity</h2><input type="email" value={email} onChange={(event) => { setEmail(event.target.value); setAvailabilityChecked(false); }} required placeholder="Email address" style={{ width: '100%', padding: 12, borderRadius: 12, border: '1px solid #cbd5e1', marginBottom: 10, boxSizing: 'border-box' }} /><input value={username} onChange={(event) => { setUsername(event.target.value); setAvailabilityChecked(false); }} required placeholder="Unique username" style={{ width: '100%', padding: 12, borderRadius: 12, border: '1px solid #cbd5e1', marginBottom: 10, boxSizing: 'border-box' }} /><input value={displayNickname} onChange={(event) => setDisplayNickname(event.target.value)} required placeholder="Display nickname" style={{ width: '100%', padding: 12, borderRadius: 12, border: '1px solid #cbd5e1', marginBottom: 10, boxSizing: 'border-box' }} /><div style={{ fontSize: 12, color: '#334155', marginBottom: 10 }}>{availabilityBusy ? 'Checking username availability…' : availabilityStatus || 'Username availability is checked in real-time.'}</div><div style={{ display: 'flex', gap: 8 }}><button type="button" onClick={() => setSignupStep(1)} style={{ flex: 1, padding: 12 }}>Back</button><button type="button" onClick={() => void validateStepTwo()} disabled={authBusy || !otpVerified} style={{ flex: 1, padding: 12, background: '#0f172a', color: '#fff' }}>{authBusy ? 'Checking…' : 'Continue'}</button></div></div>}{signupStep === 3 && <div><h2 style={{ margin: '0 0 8px', fontSize: 20 }}>Password & Consent</h2><input type="password" value={signupPassword} onChange={(event) => setSignupPassword(event.target.value)} required placeholder="Password" style={{ width: '100%', padding: 12, borderRadius: 12, border: '1px solid #cbd5e1', marginBottom: 10, boxSizing: 'border-box' }} /><input type="password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} required placeholder="Confirm password" style={{ width: '100%', padding: 12, borderRadius: 12, border: '1px solid #cbd5e1', marginBottom: 10, boxSizing: 'border-box' }} /><label style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 10, fontSize: 13, color: '#334155' }}><input type="checkbox" checked={legalAccepted} onChange={(event) => setLegalAccepted(event.target.checked)} /><span>I agree to <button type="button" onClick={() => setLegalModal('terms')} style={{ border: 'none', background: 'transparent', color: '#0284c7', textDecoration: 'underline' }}>Terms & Conditions</button> and <button type="button" onClick={() => setLegalModal('privacy')} style={{ border: 'none', background: 'transparent', color: '#0284c7', textDecoration: 'underline' }}>Privacy Policy</button></span></label><div style={{ display: 'flex', gap: 8 }}><button type="button" onClick={() => setSignupStep(2)} style={{ flex: 1, padding: 12 }}>Back</button><button type="submit" disabled={authBusy || !legalAccepted || !availabilityChecked || signupPassword !== confirmPassword} style={{ flex: 1, padding: 12, background: '#0f172a', color: '#fff', opacity: authBusy || !legalAccepted || !availabilityChecked || signupPassword !== confirmPassword ? .65 : 1 }}>{authBusy ? 'Creating…' : 'Create Account'}</button></div></div>}{authError && <div role="alert" style={{ marginTop: 12, color: '#b91c1c', fontSize: 13, fontWeight: 600 }}>{authError}</div>}<div style={{ marginTop: 14, textAlign: 'center', fontSize: 12 }}><button type="button" onClick={() => setLegalModal('terms')} style={{ border: 'none', background: 'transparent', textDecoration: 'underline' }}>Terms & Conditions</button><span style={{ margin: '0 8px' }}>|</span><button type="button" onClick={() => setLegalModal('privacy')} style={{ border: 'none', background: 'transparent', textDecoration: 'underline' }}>Privacy Policy</button></div></form>}
      </div>
    </div>
  );
}