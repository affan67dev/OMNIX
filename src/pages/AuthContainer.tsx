...
export function AuthContainer() {
  ...
  useEffect(() => {
    const bootstrap = async () => {
      try {
        ...
        let sessionUser: { id: string; email?: string; phone?: string; user_metadata?: { username?: string } } | null = null;
        try {
        ...
        setBackendWarning(backendReachable ? '' : 'Backend service is unavailable. Authentication remains available through Supabase, but some application features may be unavailable.');

        try {
          if (await hasConfiguredAppLock() && sessionUser) setAppLocked(true);
        } catch (error) {
          console.error('App lock bootstrap read failed', error);
        }

        if (sessionUser && launchPayload?.targetScreen === 'chat') {
          setActiveTab('messages'); setTargetConversationId(launchPayload.conversationId ?? '');
        } else if (sessionUser && launchPayload?.targetScreen === 'profile') {
          setActiveTab('profile');
        } else if (sessionUser && launchPayload?.targetScreen === 'settings') {
  ...
  if (screen === 'dashboard') {
    ...
    return <div style={{ background: '#000', color: '#fff', minHeight: '100vh', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', display: 'flex', flexDirection: 'column', paddingBottom: showBottomBar ? 54 : 0 }}>
      ...
      {activeTab !== 'messages' && activeTab !== 'settings' && <div style={{ height: 56, background: '#000', position: 'sticky', top: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px', borderBottom: '1px solid #121212' }}><span style={{ fontSize: 22, fontWeight: 'bold', fontStyle: 'italic', letterSpacing: '-0.5px' }}>OMNIX</span><div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>{activeTab === 'profile' ? <svg onClick={() => setActiveTab('settings')} width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ cursor: 'pointer' }}><circle cx="12" cy="12" r="3" /><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1-1V3a2 2 0 0 1 4 0v.09A1.65 1.65 0 0 0 17 4.6a1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 0 0 4h-.09a1.65 1.65 0 0 0-1.51 1z" /></svg> : <><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78a5.5 5.5 0 0 0 0-7.78z" /></svg><svg onClick={() => setActiveTab('messages')} width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ transform: 'rotate(15deg)', cursor: 'pointer' }}><line x1="22" y1="2" x2="11" y2="13" /><polygon points="22 2 15 22 11 13 2 9 22 2" /></svg></>}</div></div>}
      ...
      <div style={{ display: activeTab === 'search' ? 'block' : 'none', padding: 16 }}><input placeholder="Search secure metadata pools..." style={{ width: '100%', padding: '10px 14px', borderRadius: 8, background: '#121212', border: '1px solid #262626', color: '#fff', outline: 'none', fontSize: 13 }} /></div>
