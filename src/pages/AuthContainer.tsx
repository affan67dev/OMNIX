...
export function AuthContainer() {
  ...
  const resetSession = () => { clearCorruptLocalState(); void supabase.auth.signOut(); setScreen('login'); setActiveTab('home'); setTargetConversationId(''); setIdentifier(''); setPassword(''); setCurrentUsername('omnix_user'); setAuthError(''); setAuthBusy(false); setOfflineMode(false); setSignupStep(1); setCountryCode('+1'); setPhoneNumber(''); setOtpChallengeId(''); setOtpCode(''); setOtpVerified(false); setEmail(''); setUsername(''); setDisplayNickname(''); setAvailabilityChecked(false); setAvailabilityStatus(''); setSignupPassword(''); setConfirmPassword(''); setLegalAccepted(false); setAlgoLogs(['Algorithm Initialized: Passive State Listening...']); };
  const triggerPostPermission = () => { if (window.confirm('OMNIX System Request:\n"Allow explicit secure system permission to view native phone storage gallery content safely?"')) alert('✅ Storage pipeline connection authorized without leaking system location strings.'); };
  ...
  if (screen === 'dashboard') {
    ...
    return <div style={{ background: '#000000', color: '#ffffff', minHeight: '100vh', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', display: 'flex', flexDirection: 'column', boxSizing: 'border-box', paddingBottom: showBottomBar ? '54px' : '0px' }}>
      {activeTab !== 'messages' && activeTab !== 'settings' && <div style={{ height: '56px', backgroundColor: '#000000', position: 'sticky', top: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px', borderBottom: '1px solid #121212' }}><span style={{ fontSize: '22px', fontWeight: 'bold', fontStyle: 'italic', letterSpacing: '-0.5px' }}>OMNIX</span><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78" /></svg><svg onClick={() => setActiveTab('messages')} width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ transform: 'rotate(15deg)', cursor: 'pointer' }}><line x1="22" y1="2" x2="11" y2="13" /><polygon points="22 2 15 22 23 7" /></svg></div></div>}
      ...
      {showBottomBar && <div style={{ height: '50px', backgroundColor: '#000000', borderTop: '1px solid #121212', position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'space-around' }}><div onClick={() => setActiveTab('home')} style={{ cursor: 'pointer', opacity: activeTab === 'home' ? 1 : 0.4 }}><span aria-label="Home">⌂</span></div><div onClick={() => setActiveTab('search')} style={{ cursor: 'pointer', opacity: activeTab === 'search' ? 1 : 0.4 }}><span aria-label="Search">⌕</span></div><div onClick={triggerPostPermission} style={{ cursor: 'pointer', opacity: 0.8 }}><span aria-label="Post">＋</span></div><div onClick={() => setActiveTab('reels')} style={{ cursor: 'pointer', opacity: activeTab === 'reels' ? 1 : 0.4 }}><span aria-label="Reels">▶</span></div><div onClick={() => setActiveTab('profile')} style={{ cursor: 'pointer', opacity: activeTab === 'profile' ? 1 : 0.4 }}><span aria-label="Profile">◯</span></div></div>}
  ...
  return <div style={{ background: 'linear-gradient(130deg, #f0f9ff 0%, #ecfeff 34%, #fdf2f8 68%, #fef9c3 100%)', color: '#111827', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between', padding: '30px 20px', boxSizing: 'border-box' }}>
    ...
    <div style={{ width: '100%', maxWidth: '420px' }}><div style={{ backgroundColor: 'rgba(255, 255, 255, 0.9)', backdropFilter: 'blur(14px)', border: '1px solid #e2e8f0', padding: '24px', borderRadius: '24px', boxSizing: 'border-box', boxShadow: '0 20px 50px rgba(30, 41, 59, 0.12)' }}>
      {screen === 'login' ? <form onSubmit={handleLogin}><h2 style={{ margin: '0 0 12px', fontSize: '22px', fontWeight: 800 }}>Login</h2><input value={identifier} onChange={event => setIdentifier(event.target.value)} required placeholder="Identifier (email or phone)" style={{ width: '100%', padding: '12px', borderRadius: '12px', border: '1px solid #cbd5e1', marginBottom: '12px', boxSizing: 'border-box' }} /><input type="password" value={password} onChange={event => setPassword(event.target.value)} required placeholder="Password" style={{ width: '100%', padding: '12px', borderRadius: '12px', border: '1px solid #cbd5e1', marginBottom: '14px', boxSizing: 'border-box' }} /><button type="button" onClick={() => { window.location.href = '/forgot-password'; }} style={{ border: 'none', background: 'transparent', color: '#0369a1', cursor: 'pointer', textDecoration: 'underline', fontSize: '13px', padding: 0, marginBottom: '14px' }}>Forgot Password?</button><button type="submit" disabled={authBusy} style={{ width: '100%', padding: '12px', borderRadius: '12px', border: 'none', background: '#0f172a', color: '#fff', fontWeight: 700, cursor: authBusy ? 'not-allowed' : 'pointer', opacity: authBusy ? 0.7 : 1 }}>{authBusy ? 'Signing in…' : 'Login'}</button><div style={{ marginTop: '14px', textAlign: 'center', fontSize: '13px', color: '#475569' }}>New here? <button type="button" onClick={() => { setScreen('signup'); setAuthError(''); }} style={{ border: 'none', background: 'transparent', color: '#0369a1', textDecoration: 'underline', cursor: 'pointer', padding: 0, fontWeight: 700 }}>Sign Up</button></div></form> : <form onSubmit={createAccount}><div style={{ marginBottom: '10px' }}><button type="button" onClick={() => { setScreen('login'); setAuthError(''); }} style={{ borderRadius: '999px', border: '1px solid #cbd5e1', background: '#ffffff', color: '#334155', padding: '8px 12px', cursor: 'pointer', fontWeight: 700 }}>Back to Login</button></div><div style={{ fontSize: '12px', color: '#0369a1', textTransform: 'uppercase', letterSpacing: '0.14em', marginBottom: '8px' }}>Step {signupStep} of 3</div>{signupStep === 1 && <div><h2 style={{ margin: '0 0 8px', fontSize: '20px', fontWeight: 800 }}>What is your phone number?</h2><div style={{ display: 'grid', gridTemplateColumns: '100px 1fr', gap: '8px', marginBottom: '10px' }}><input value={countryCode} onChange={event => setCountryCode(event.target.value)} placeholder="+1" style={{ padding: '12px', borderRadius: '12px', border: '1px solid #cbd5e1' }} /><input value={phoneNumber} onChange={event => setPhoneNumber(event.target.value)} placeholder="Please enter your phone number" style={{ padding: '12px', borderRadius: '12px', border: '1px solid #cbd5e1' }} /></div><button type="button" onClick={() => void requestOtp()} disabled={authBusy} style={{ width: '100%', padding: '12px', borderRadius: '12px', border: 'none', background: '#0f172a', color: '#fff', fontWeight: 700, cursor: authBusy ? 'not-allowed' : 'pointer' }}>{authBusy ? 'Sending OTP…' : 'Send OTP'}</button>{otpChallengeId ? <div style={{ marginTop: '14px' }}><OtpInput onComplete={value => setOtpCode(value)} /><button type="button" onClick={() => void verifyOtp()} disabled={authBusy || otpCode.length !== 6} style={{ width: '100%', marginTop: '10px', padding: '12px', borderRadius: '12px', border: '1px solid #94a3b8', background: '#f8fafc', color: '#0f172a', fontWeight: 700 }}>{authBusy ? 'Verifying…' : 'Verify OTP'}</button></div> : null}</div>}{signupStep === 2 && <div><h2 style={{ margin: '0 0 8px', fontSize: '20px', fontWeight: 800 }}>Profile Identity</h2><input type="email" value={email} onChange={event => { setEmail(event.target.value); setAvailabilityChecked(false); }} required placeholder="Email address" style={{ width: '100%', padding: '12px', borderRadius: '12px', border: '1px solid #cbd5e1', marginBottom: '10px', boxSizing: 'border-box' }} /><input value={username} onChange={event => { setUsername(event.target.value); setAvailabilityChecked(false); }} required placeholder="Unique username" style={{ width: '100%', padding: '12px', borderRadius: '12px', border: '1px solid #cbd5e1', marginBottom: '10px', boxSizing: 'border-box' }} /><input value={displayNickname} onChange={event => setDisplayNickname(event.target.value)} required placeholder="Display nickname" style={{ width: '100%', padding: '12px', borderRadius: '12px', border: '1px solid #cbd5e1', marginBottom: '10px', boxSizing: 'border-box' }} /><div style={{ fontSize: '12px', color: '#334155', marginBottom: '10px' }}>{availabilityBusy ? 'Checking username availability…' : availabilityStatus || 'Username availability is checked in real-time.'}</div><div style={{ display: 'flex', gap: '8px' }}><button type="button" onClick={() => setSignupStep(1)} style={{ flex: 1, padding: '12px', borderRadius: '12px', border: '1px solid #cbd5e1', background: '#ffffff' }}>Back</button><button type="button" onClick={() => void validateStepTwo()} disabled={authBusy || !otpVerified} style={{ flex: 1, padding: '12px', borderRadius: '12px', border: 'none', background: '#0f172a', color: '#fff', fontWeight: 700 }}>{authBusy ? 'Checking…' : 'Continue'}</button></div></div>}{signupStep === 3 && <div><h2 style={{ margin: '0 0 8px', fontSize: '20px', fontWeight: 800 }}>Password & Consent</h2><input type="password" value={signupPassword} onChange={event => setSignupPassword(event.target.value)} required placeholder="Password" style={{ width: '100%', padding: '12px', borderRadius: '12px', border: '1px solid #cbd5e1', marginBottom: '10px', boxSizing: 'border-box' }} /><input type="password" value={confirmPassword} onChange={event => setConfirmPassword(event.target.value)} required placeholder="Confirm password" style={{ width: '100%', padding: '12px', borderRadius: '12px', border: '1px solid #cbd5e1', marginBottom: '10px', boxSizing: 'border-box' }} /><label style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', marginBottom: '10px', fontSize: '13px', color: '#334155' }}><input type="checkbox" checked={legalAccepted} onChange={event => setLegalAccepted(event.target.checked)} style={{ marginTop: '2px' }} /><span>I agree to <button type="button" onClick={() => setLegalModal('terms')} style={{ border: 'none', background: 'transparent', color: '#0284c7', cursor: 'pointer', padding: 0, textDecoration: 'underline' }}>Terms & Conditions</button> and <button type="button" onClick={() => setLegalModal('privacy')} style={{ border: 'none', background: 'transparent', color: '#0284c7', cursor: 'pointer', padding: 0, textDecoration: 'underline' }}>Privacy Policy</button></span></label><div style={{ display: 'flex', gap: '8px' }}><button type="button" onClick={() => setSignupStep(2)} style={{ flex: 1, padding: '12px', borderRadius: '12px', border: '1px solid #cbd5e1', background: '#ffffff' }}>Back</button><button type="submit" disabled={authBusy || !legalAccepted || !availabilityChecked || signupPassword !== confirmPassword} style={{ flex: 1, padding: '12px', borderRadius: '12px', border: 'none', background: '#0f172a', color: '#fff', fontWeight: 700, opacity: authBusy ? 0.7 : 1 }}>{authBusy ? 'Creating…' : 'Create Account'}</button></div></div>}{authError && <div role="alert" style={{ marginTop: '12px', color: '#b91c1c', fontSize: '13px', fontWeight: 600 }}>{authError}</div>}<div style={{ marginTop: '14px', textAlign: 'center', fontSize: '12px', color: '#475569' }}><button type="button" onClick={() => setLegalModal('terms')} style={{ border: 'none', background: 'transparent', color: '#334155', cursor: 'pointer', textDecoration: 'underline' }}>Terms & Conditions</button><span style={{ margin: '0 8px', color: '#94a3b8' }}>|</span><button type="button" onClick={() => setLegalModal('privacy')} style={{ border: 'none', background: 'transparent', color: '#334155', cursor: 'pointer', textDecoration: 'underline' }}>Privacy Policy</button></div></form>}
    </div></div>
