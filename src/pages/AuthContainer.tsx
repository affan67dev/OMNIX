...
export function AuthContainer() {
  ...
  const clearCorruptLocalState = () => {
    try {
      ...
      console.error('Failed to clear corrupt local authentication state', error);
    }
  };
  ...
  useEffect(() => {
    const bootstrap = async () => {
      try {
        ...
        try { storedUser = window.localStorage.getItem('user'); } catch (error) { console.error('Bootstrap local storage read failed', error); }
  ...
  if (screen === 'dashboard') {
    ...
    return <div style={{ background: '#000000', color: '#ffffff', minHeight: '100vh', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', display: 'flex', flexDirection: 'column', paddingBottom: showBottomBar ? '54px' : '0px' }}>
      {activeTab !== 'messages' && activeTab !== 'settings' && <div style={{ height: 56, background: '#000', position: 'sticky', top: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px', borderBottom: '1px solid #121212' }}><span style={{ fontSize: 22, fontWeight: 'bold', fontStyle: 'italic' }}>OMNIX</span><div style={{ display: 'flex', alignItems: 'center', gap: 12 }}><span>♡</span><span onClick={() => setActiveTab('messages')} style={{ cursor: 'pointer' }}>➤</span></div></div>}
      ...
      {showBottomBar && <div style={{ height: 50, background: '#000', borderTop: '1px solid #121212', position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'space-around' }}><div onClick={() => setActiveTab('home')} style={{ cursor: 'pointer', opacity: activeTab === 'home' ? 1 : .4 }}>⌂</div><div onClick={() => setActiveTab('search')} style={{ cursor: 'pointer', opacity: activeTab === 'search' ? 1 : .4 }}>⌕</div><div onClick={triggerPostPermission} style={{ cursor: 'pointer', opacity: .8 }}>＋</div><div onClick={() => setActiveTab('reels')} style={{ cursor: 'pointer', opacity: activeTab === 'reels' ? 1 : .4 }}>▶</div><div onClick={() => setActiveTab('profile')} style={{ cursor: 'pointer', opacity: activeTab === 'profile' ? 1 : .4 }}>◯</div></div>}
    </div>;
  }

  const finalizeAuth = (usernameValue: string) => { setCurrentUsername(usernameValue || 'omnix_user'); setIdentifier(usernameValue || 'omnix_user'); setScreen('dashboard'); setActiveTab('home'); setAuthError(''); };
